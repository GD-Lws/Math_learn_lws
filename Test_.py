...
def dijkstra(graph_array, sb, db):
    n = len(graph_array)
    visit = [0] * n
    dis = [inf] * n
    dis[0] = 0
    ...
    parent = [0] * n  # 前驱顶点的初始化
    for i in range(n):
        temp_id = [index for index, value in enumerate(visit) if value == 0]  # 查找未标号的顶点
        for v in temp_id:
            # 更新当前点的最短路径
            if graph_array[u][v] + dis[u] < dis[v]:
                dis[v] = dis[u] + graph_array[u][v]
                parent[v] = u
        temp = dis
        temp[visit == 1] = inf  # 已标号点的距离换成无穷
        t = np.min(temp)
        u = temp.index(t)  # 找标号值最小的顶点
        visit[u] = 1  # 标记已经标号的顶点
        mypath = []
        if parent[db] != 0:  # 存在路
            t = db
